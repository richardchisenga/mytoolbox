...
export default function AdminDashboard() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setError(e.message || "Administrator access required.");
    }
  };
  ...
  const fetchAllStats = async () => {
    ...
    try {
      ...
      const [statsRes, usersRes] = await Promise.all([
        ...
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/admin/users/detailed`, { headers }),
      ]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white p-8 rounded-xl shadow-lg max-w-sm w-full border border-gray-200 text-center">
          ...
          <button onClick={handleLogin} className="mt-4 bg-primary text-white px-6 py-3 rounded-lg hover:opacity-90 w-full font-semibold transition-colors">
