  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const [lessonsResponse, schemesResponse] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/lessons/mine`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/schemes/mine`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);

        if (!lessonsResponse.ok || !schemesResponse.ok) {
          throw new Error("Failed to load dashboard records.");
        }

        const lessonData = await lessonsResponse.json();
        const schemeData = await schemesResponse.json();
        const userLessons = Array.isArray(lessonData) ? lessonData : [];
        const userSchemes = Array.isArray(schemeData) ? schemeData : [];
