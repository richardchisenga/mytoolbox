...
export default function NotesPage() {
  ...
  const readFile = (file: File) => {
    ...
    if (!allowed) { setError("For browser upload, use TXT, MD, CSV or JSON notes. PDF/DOCX extraction can be added with a document parser later."); return; }
    const reader = new FileReader();
    reader.onload = () => { setContent(String(reader.result || "")); if (!title) setTitle(file.name.replace(/\.[^.]+$/, "")); setMessage(`Loaded ${file.name}. Review the content and click Save Note.`); };
