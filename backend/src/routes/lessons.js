...
router.post('/generate', authenticate, async (req, res) => {
  try {
    ...
    // Build prompt based on curriculum type
    let prompt;
    if (curriculumType === 'obc') {
      prompt = buildOBCPrompt(grade, subject, topic, size, boys, girls);
    } else {
      prompt = buildCBCPrompt(grade, subject, topic, size, boys, girls);
    }
    ...
    // Fallback mock lesson based on curriculum type
    let mockLesson;
    if (curriculumType === 'obc') {
      mockLesson = generateOBCMockLesson(grade, subject, topic, size, boys, girls);
    } else {
      mockLesson = generateCBCMockLesson(grade, subject, topic, size, boys, girls);
    }
...
function buildCBCPrompt(grade, subject, topic, size, boys, girls) {
  return `
...
function buildOBCPrompt(grade, subject, topic, size, boys, girls) {
  return `
...
function generateCBCMockLesson(grade, subject, topic, size, boys, girls) {
  return {
    id: `lesson-${Date.now()}`,
    userId: 'mock-user',
    title: topic,
...
function generateOBCMockLesson(grade, subject, topic, size, boys, girls) {
  return {
    id: `lesson-${Date.now()}`,
    userId: 'mock-user',
    title: topic,
